...
$govRoot = Resolve-Path (Join-Path $PSScriptRoot "..")

# Project root: git root if available, else fall back to governance root
# This supports both flat layout (Forge IS the git root) and subfolder layout (Forge/ inside project root)
try {
  $gitRoot = (& git rev-parse --show-toplevel 2>$null)
  if ($LASTEXITCODE -eq 0 -and $gitRoot) {
    $root = Resolve-Path $gitRoot.Trim()
  } else {
    $root = $govRoot
  }
} catch {
...
try {
  ...
  Append-TestRunLog -root $govRoot -statusText $statusText -runtimePath $runtimePath -startUtc $startUtc -endUtc $endUtc `
  ...
  Write-TestRunLatest -root $govRoot -statusText $statusText -runtimePath $runtimePath -startUtc $startUtc -endUtc $endUtc `
    -exitCodes $exitCodes -summaries $summaries `
