...
# failures when overwrite_diff_log writes IN_PROCESS entries.
...
try {
  ...
  # Governance root: parent of the scripts/ dir (where Contracts/, evidence/ live)
  # This supports both flat layout (Forge IS the git root) and subfolder layout (Forge/ inside project root)
  ...
  try {
    ...
    if (-not (Test-Path $boundariesJson)) {
      ...
      foreach ($layer in $boundaries.layers) {
        ...
        # Resolve glob to actual files (source code is at project root)
        $globPath = Join-Path $projectRoot $layerGlob
  ...
  # ── A5: Diff Log Gate (non-blocking -- moved to warnings) ─────────────
  # Missing file -> FAIL (blocking). TODO placeholders -> WARN (non-blocking)
  # to prevent cyclic failures when overwrite_diff_log writes IN_PROCESS entries
  # that the watch-audit picks up before the builder can finalise.
  ...
  try {
    ...
    $warnings["A5"] = "WARN -- Error checking diff log: $_"
  }
  ...
  try {
    ...
    if (-not (Test-Path $forgeJson)) {
      ...
      $depFilePath = Join-Path $projectRoot $depFile
            ...
            switch ($lang) {
              "python" {
                ...
                $stdlibModules = @(
                  ...
                  "random", "re", "readline", "reprlib", "secrets", "select",
                  "selectors", "shelve", "shlex", "shutil", "signal", "site",
                  "socket", "socketserver", "sqlite3", "ssl", "stat",
                  "statistics", "string", "struct", "subprocess", "sys",
                  "sysconfig", "tempfile", "textwrap", "threading", "time",
                  "timeit", "token", "tokenize", "tomllib", "traceback",
                  "tracemalloc", "types", "typing", "unicodedata", "unittest",
                ...
                # Check if it's a local directory (at project root)
                $localDir = Join-Path $projectRoot $imp
    ...
    if (-not (Test-Path $physicsYaml)) {
      ...
      if (Test-Path $forgeJson) {
        ...
        switch ($forge.backend.language) {
          "python" { $routerDirs = @(Join-Path $projectRoot "app/api/routers") }
          "typescript" { $routerDirs = @((Join-Path $projectRoot "src/routes"), (Join-Path $projectRoot "src/controllers")) }
          "go" { $routerDirs = @((Join-Path $projectRoot "handlers"), (Join-Path $projectRoot "api")) }
        }
      } else {
        # Fallback: check common paths
        $routerDirs = @(
          (Join-Path $projectRoot "app/api/routers"),
          (Join-Path $projectRoot "src/routes"),
          (Join-Path $projectRoot "handlers")
        )
      }
