...
class TestRunSubAgent:
    ...
    @pytest.mark.asyncio
    async def test_coder_uses_tool_loop(self):
        """CODER goes through tool loop like all other roles (pull-first)."""
        from app.clients.agent_client import ToolCall
        ...
        async def fake_stream(*args, **kwargs):
            ...
            # Coder now receives tools (pull-first model)
            assert kwargs.get("tools") is not None, "CODER should receive tools"
            if call_count == 0:
                call_count += 1
