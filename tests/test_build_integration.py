...
def _mock_stream_agent(responses: list[str]):
    ...
    async def mock_stream(*, api_key, model, system_prompt, messages, max_tokens=16384, usage_out, tools=None, on_retry=None, token_limiter=None, key_pool=None):
        nonlocal call_count
        idx = call_count
        call_count += 1
...
class TestInterjection:
    ...
    async def test_interjection_injected_into_conversation(self):
        ...
        try:
            ...
            async def capturing_stream(*, api_key, model, system_prompt, messages, max_tokens=16384, usage_out, tools=None, on_retry=None, token_limiter=None, key_pool=None):
